...
use App\Form\FormulaireFilterType;
...
final class FormulaireController extends AbstractController
{
    ...
    public function index(Request $request, FormulaireRepository $formulaireRepository, PaginatorInterface $paginator): Response
    {
        ...
        $filterForm = $this->createForm(FormulaireFilterType::class);
        $filterForm->handleRequest($request);

        $isFilterSubmitted = $filterForm->isSubmitted() && $filterForm->isValid();
        $filters = [];
        ...
        // Получаем query с учетом фильтров
        $query = $formulaireRepository->createQueryForUserWithFilters($user, $filters)->getQuery();
